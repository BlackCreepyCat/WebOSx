        ...
        //
        // Mode "économie d'énergie" (Réglages > Performance) : réduit dynamiquement le
        // nombre de particules dessinées par le shader (uOrbCount / uSparkleCount, via un
        // early-break dans les boucles GLSL) et la résolution interne du canvas. Piloté en
        // continu depuis window.__waveConfig.lowPower, donc bascule sans recharger la page.
        (function() {
            ...
            // Mode économie d'énergie : facteur multiplicatif supplémentaire sur la
            // résolution interne, indépendant du mobile (peut se cumuler). 0.55 ici
            // équivaut à un peu moins d'un tiers du nombre de pixels calculés par
            ...
            function resize() {
                ...
                const extraScale = lowPowerActive ? LOW_POWER_RES_SCALE : 1;
                canvas.width = Math.max(1, Math.round(W * dpr * renderScale * extraScale));
                canvas.height = Math.max(1, Math.round(H * dpr * renderScale * extraScale));
            ...
            function getWaveConfig() {
                return window.__waveConfig || { speed: 1, width: 1, enabled: true, lowPower: false };
            }
...
void main() {
  ...
  // uOrbCount permet de tronquer la boucle (early-break) en mode économie
  // d'énergie : moins d'itérations par pixel = moins de coût GPU, sans
  // recompiler le shader (la borne littérale de la boucle GLSL reste fixe).
  ...
  // ── Étincelles rapides ──
  // Même principe de troncature via uSparkleCount.
  ...
  gl_FragColor = vec4(color, 1.0);
            ...
            const u = {};
            ['uTime', 'uResolution', 'uBgColor', 'uWaveColor', 'uHighlight', 'uGlowColor', 'uParticleColor', 'uWidthScale', 'uOrbCount', 'uSparkleCount'].forEach(n => {
                u[n] = gl.getUniformLocation(prog, n);
            });

            // Nombre de particules par mode. En low power : 6 orbes (au lieu de 20) et
            // 18 étincelles (au lieu de 120), soit environ 85% d'itérations en moins
            // dans les deux boucles per-pixel les plus coûteuses du shader.
            const PARTICLE_COUNTS = {
                normal: { orbs: 20, sparkles: 120 },
                lowPower: { orbs: 6, sparkles: 18 }
            };
            ...
            function frame(ts) {
                ...
                if (cfg.enabled !== false) {
                    ...
                    // Bascule low power : ne redimensionne le canvas que sur changement
                    // d'état (pas à chaque frame), pour éviter un resize coûteux inutile.
                    const wantsLowPower = cfg.lowPower === true;
                    if (wantsLowPower !== lowPowerActive) { lowPowerActive = wantsLowPower; resize(); }
                    ...
                    const counts = wantsLowPower ? PARTICLE_COUNTS.lowPower : PARTICLE_COUNTS.normal;
                    ...
                    gl.uniform1f(u.uOrbCount, counts.orbs);
                    gl.uniform1f(u.uSparkleCount, counts.sparkles);

        ...
        I18N.registerLang('de', {
            ...
            'mine.lastTime': 'Letzte Zeit',
            'mine.none': '—',
            'mine.flagHint': 'Rechtsklick: Fahne · Doppelklick auf Zahl: Nachbarn aufdecken'
        });
        I18N.registerLang('it', {
            'app.minesweeper': 'Campo minato',
            'mine.difficulty.beginner': 'Principiante',
            'mine.difficulty.intermediate': 'Intermedio',
            'mine.difficulty.expert': 'Esperto',
            'mine.newGame': 'Nuova partita',
            'mine.mines': 'Mine',
            'mine.time': 'Tempo',
            'mine.win': 'Hai vinto! 🎉',
            'mine.lose': 'Boom! 💥',
            'mine.bestTime': 'Miglior tempo',
            'mine.lastTime': 'Ultimo tempo',
            'mine.none': '—',
            'mine.flagHint': 'Clic destro: bandierina · Doppio clic su un numero: rivela le celle vicine'
        });
        I18N.registerLang('pt', {
            'app.minesweeper': 'Campo minado',
            'mine.difficulty.beginner': 'Principiante',
            'mine.difficulty.intermediate': 'Intermédio',
            'mine.difficulty.expert': 'Perito',
            'mine.newGame': 'Novo jogo',
            'mine.mines': 'Minas',
            'mine.time': 'Tempo',
            'mine.win': 'Ganhou! 🎉',
            'mine.lose': 'Bum! 💥',
            'mine.bestTime': 'Melhor tempo',
            'mine.lastTime': 'Último tempo',
            'mine.none': '—',
            'mine.flagHint': 'Clique direito: bandeira · Duplo clique num número: revelar vizinhos'
        });
        ...
        (function injectMinesweeperStyle() {
            ...
            style.textContent = `
                ...
                .ms-last { font-size:11px; color:var(--text-secondary, #999); white-space:nowrap; margin-right:12px; }
        ...
        OS.registry.register({
            ...
            icon: MINESWEEPER_ICON, defaultWidth: 580, defaultHeight: 640,
            init: (c, api) => {
                c.innerHTML = `
                <div class="ms-container">
                    <div class="ms-toolbar">
                        ...
                        <span class="ms-last" id="ms-last-label"></span>
                ...
                const lastLabelEl = c.querySelector('#ms-last-label');
                ...
                let lastElapsed = null; // dernière partie terminée (gagnée ou perdue), affiché quel que soit le résultat
                ...
                function updateLastLabel() {
                    lastLabelEl.textContent = I18N.t('mine.lastTime') + ' : ' + (lastElapsed !== null ? lastElapsed + 's' : I18N.t('mine.none'));
                }
                ...
                function buildBoard() {
                    // Fusionne avec localStorage à chaque partie (garde le meilleur des
                    // deux par difficulté) au lieu d'écraser bestTimes : certains
                    // navigateurs restreignent/sandboxent localStorage en file://, donc
                    // un rechargement qui remplacerait purement et simplement la valeur
                    // en mémoire pourrait effacer un record tout juste gagné si l'écriture
                    // n'a pas réellement persisté.
                    const freshBest = msLoadBest();
                    Object.keys(freshBest).forEach(k => {
                        if (bestTimes[k] === undefined || freshBest[k] < bestTimes[k]) bestTimes[k] = freshBest[k];
                    });
                    ...
                    stopTimer(); updateTimer(); updateMinesLeft(); updateBestLabel(); updateLastLabel();
                    faceEl.textContent = '🙂';
